    ...
    def increment(self,courier_wait_time, order_wait_time,writer):
        with self.lock:
            ...
            if self.count == self.total_orders:  
                ...
                self.c_avg = self.courier_wait_time/self.total_orders
                self.o_avg = self.food_wait_time/self.total_orders
                text = f'----------------------------------------------------------------------------------------------------------------------------\nAverage Courier Wait Time: {self.c_avg} ms\nAverage Order Wait Time: {self.o_avg} ms'
                writer.write(text)
                print(text)
...
def prepareOrder(placed_orders, ready_orders, writer):
    ...
    order.setTimeReady(time_ready)
    text = f'Order {order.getID()} ({order.getName()}) is ready for pick up! ({datetime.fromtimestamp(time_ready)})\n'
    writer.write(text)
    print(text)

# def placeOrder(new_order, placed_orders, writer):
...
def read_entries(data, index, ready_orders, placed_orders, order_counter,writer):
    ...
    for i in range(entries_to_read):
        ...
        order_prepTime = int(order['prepTime'])
        new_order = Order(order_id, order_name, order_prepTime)
        print(f'preptime for {new_order.getName()}: {new_order.getPrepTime()}')

        # PLACING ORDER
        #placeOrder(new_order,placed_orders,writer)
        placed_orders.put(new_order)
        time_order_placed = time.time()
        text = f'Order {new_order.getID()} ({new_order.getName()}) received! ({datetime.fromtimestamp(time_order_placed)})\n'
        writer.write(text)
        ...
        #Preparing Order
        threading.Timer(new_order.getPrepTime(), prepareOrder, args=(placed_orders,ready_orders,writer,)).start()
